...
export function scoreReasoningComposability(
  ...
  if (previousReasonings.length > 0) {
    ...
    for (const prev of previousReasonings) {
      ...
      const overlap = countByCondition(prevWords, (w) => currentWords.has(w));
...
export function gradeTrade(input: {
  ...
  const tradeId = `v37_${Date.now()}_${Math.random().toString(36).slice(ID_RANDOM_START, ID_RANDOM_START + ID_RANDOM_LENGTH_SHORT)}`;
