...
/**
 * Average Rating Display Precision
 *
 * Number of decimal places shown when displaying aggregate strategy ratings.
 * The 1-5 star rating average is stored and displayed to 2 decimal places
 * (e.g., "4.27 stars") to convey meaningful precision without false accuracy.
 *
 * Formula: avgRating = (sum / count).toFixed(AVG_RATING_DISPLAY_DECIMALS)
 * Example: ratings [4, 5, 4] → 13 / 3 = 4.333... → "4.33"
 *
 * Used in: rateStrategy() average rating calculation
 */
const AVG_RATING_DISPLAY_DECIMALS = 2;

/**
 * Strategy Signal Expiry Duration in Milliseconds
 *
 * Time-to-live for entry signals generated by strategy rules. After this
 * duration, signals are considered stale and should not influence trading.
 * 1 hour = 3,600,000 milliseconds matches typical intraday trading cycles.
 *
 * Formula: expiresAt = Date.now() + SIGNAL_EXPIRY_DURATION_MS
 * Example: Signal generated at 10:30 AM → expires at 11:30 AM
 *
 * Used in: generateStrategySignals() entry rule signal creation
 * Note: Exit signals (stop loss, take profit) do not set expiresAt
 */
const SIGNAL_EXPIRY_DURATION_MS = 60 * 60 * 1000;

/**
 * Performance Value Rounding Precision
 *
 * Multiplier and divisor pair for rounding best/worst performance values
 * to 2 decimal places in strategy leaderboard entries.
 *
 * Formula: Math.round(value * PERFORMANCE_ROUNDING_MULTIPLIER) / PERFORMANCE_ROUNDING_MULTIPLIER
 * Example: 12.3456 → Math.round(1234.56) / 100 = 12.35
 *
 * Used in: getStrategyLeaderboard() bestPerformance and worstPerformance
 */
  ...
  const avgRating =
    totalRatings > 0
      ? (
        ...
        ).toFixed(AVG_RATING_DISPLAY_DECIMALS)
      : "0";
...
export async function getStrategyLeaderboard() {
  ...
  for (const strategy of allStrategies) {
    ...
    leaderboard.push({
      ...
      bestPerformance:
        performances.length > 0
          ? Math.round((findMax(performances.map((p: number) => ({ value: p })), 'value')?.value ?? 0) * PERFORMANCE_ROUNDING_MULTIPLIER) / PERFORMANCE_ROUNDING_MULTIPLIER
          : 0,
      worstPerformance:
        performances.length > 0
          ? Math.round((findMin(performances.map((p: number) => ({ value: p })), 'value')?.value ?? 0) * PERFORMANCE_ROUNDING_MULTIPLIER) / PERFORMANCE_ROUNDING_MULTIPLIER
          : 0,
      totalTrades,
    });
  }
    ...
    for (const rule of params.entryRules) {
      ...
      if (triggered && strength > SIGNAL_STRENGTH_MIN_THRESHOLD) {
        const metadata: SignalMetadata = {
          ...
          expiresAt: new Date(Date.now() + SIGNAL_EXPIRY_DURATION_MS).toISOString(),
        };
...
export async function getMarketplaceStats(): Promise<MarketplaceStats> {
  ...
  const avgPerformance =
    performances.length > 0
      ? Math.round(
          (performances.reduce((s: number, p: number) => s + p, 0) / performances.length) * PERFORMANCE_ROUNDING_MULTIPLIER,
        ) / PERFORMANCE_ROUNDING_MULTIPLIER
      : 0;
  ...
  const avgRating =
    ratingValues.length > 0
      ? Math.round(
          (ratingValues.reduce((s: number, r: number) => s + r, 0) / ratingValues.length) * PERFORMANCE_ROUNDING_MULTIPLIER,
        ) / PERFORMANCE_ROUNDING_MULTIPLIER
      : 0;
