...
| `get_execution_quote` | Get actual execution price with slippage & price impact for a specific trade | **Use BEFORE large trades (>$3) or illiquid stocks** to avoid execution losses. Returns: `{effectivePrice, midMarketPrice, priceImpactPercent, slippageBps, note}`. Example: `get_execution_quote({"symbol": "TSLAx", "side": "buy", "amount": 5})` → shows if $5 buy would have 2% slippage. **RULE:** If price impact >1%, reduce trade size or skip. **When to use:** (1) Any trade >$3 USDC, (2) Low-volume stocks (<$500k/day), (3) After seeing wide bid-ask spreads. **Prevents:** Buying at $175 mid-market but actually filling at $177 (1.1% slippage = instant -$0.20 loss on $5 trade). |
