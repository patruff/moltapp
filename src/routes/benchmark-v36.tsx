/**
 * Benchmark V36 Dashboard — 32-Dimension AI Trading Benchmark
 *
 * NEW in v36: Reasoning Auditability + Decision Reversibility
 *
 * Sections:
 ...
 * 6. Footer
 *
 * GET /benchmark-v36 — Full benchmark dashboard
 */
...
const DIMS: DimDef[] = [
  ...
  { key: "reasoningAuditability", label: "Auditability", cat: "Reasoning Quality", isNew: true },
  { key: "decisionReversibility", label: "Reversibility", cat: "Reasoning Quality", isNew: true },
...
benchmarkV36Routes.get("/", (c) => {
  ...
  /* API table */
  const apis = [
    ["/api/v1/benchmark-v36/leaderboard", "Ranked agents by 32-dimension composite score (JSON)"],
    ["/api/v1/benchmark-v36/trade-grades", "Individual trade quality assessments with auditability + reversibility"],
    ["/api/v1/benchmark-v36/dimensions", "All 32 dimension definitions and weights"],
    ["/api/v1/benchmark-v36/auditability/:agentId", "Per-agent reasoning auditability analysis"],
    ["/api/v1/benchmark-v36/reversibility/:agentId", "Per-agent decision reversibility analysis"],
    ["/api/v1/benchmark-v36/reasoning-profile", "Cross-agent reasoning quality comparison"],
    ["/api/v1/benchmark-v36/export/jsonl", "Full dataset export (JSONL for HuggingFace)"],
    ["/api/v1/benchmark-v36/export/csv", "Full dataset export (CSV)"],
    ["/api/v1/benchmark-v36/health", "Engine health check"],
  ];
...
<div class="wrap">
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;margin-bottom:22px">
    ...
    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
      ...
      <a href="/api/v1/benchmark-v36/export/jsonl" style="background:#1a1a2e;padding:8px 14px;border-radius:8px;font-size:0.82em;text-decoration:none">JSONL</a>
      <a href="/api/v1/benchmark-v36/export/csv" style="background:#1a1a2e;padding:8px 14px;border-radius:8px;font-size:0.82em;text-decoration:none">CSV</a>
    </div>
  </div>
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;margin-bottom:22px">
    ...
    <div class="stat"><div class="stat-v">${lr ? lr.avgReasoningAuditability.toFixed(0) : "--"}</div><div class="stat-l">Avg Auditability <span class="new-badge">NEW</span></div></div>
    <div class="stat"><div class="stat-v">${lr ? lr.avgDecisionReversibility.toFixed(0) : "--"}</div><div class="stat-l">Avg Reversibility <span class="new-badge">NEW</span></div></div>
  </div>
  ...
  <div class="card">
    ...
    <p style="color:#555;font-size:0.78em;margin-bottom:14px">Per-agent scores grouped by category. <span class="new-badge">NEW</span> = v36 dimensions.</p>
  ...
  <div class="grid2">
    <div class="card">
      ...
      <p style="color:#555;font-size:0.75em;margin-bottom:8px">Last 10 trades | <a href="/api/v1/benchmark-v36/trade-grades">Full JSON</a></p>
    ...
    <div>
      <div class="card" style="margin-bottom:18px">
        ...
        <div style="margin-bottom:14px">
          <h3 style="color:#ffd700;font-size:0.92em">Reasoning Auditability <span class="new-badge">NEW</span></h3>
          <p style="color:#999;font-size:0.82em;margin:5px 0">
            Can every claim be independently verified by a third party? Measures
            evidence specificity, falsifiability of predictions, verifiable reference
            density, audit trail completeness, and data provenance clarity.
          </p>
          <div style="background:#111;border-radius:6px;padding:8px;font-size:0.8em;color:#aaa">
            <strong style="color:#00ff88">High (70+):</strong> "NVDA Q4 revenue was $22.1B (source: SEC 10-K). If next-quarter guidance &lt; $24B, thesis invalidated. Current P/E of 65x implies &gt;30% earnings growth priced in."<br>
            <strong style="color:#ff4444">Low (&lt;30):</strong> "Stock looks undervalued and should go up soon."
          </div>
        </div>
        <div>
          <h3 style="color:#ffd700;font-size:0.92em">Decision Reversibility <span class="new-badge">NEW</span></h3>
          <p style="color:#999;font-size:0.82em;margin:5px 0">
            Does the agent plan for when its thesis breaks? Measures explicit exit
            conditions, thesis invalidation criteria, contingency planning, position
            unwinding logic, and risk-reward framing.
          </p>
          <div style="background:#111;border-radius:6px;padding:8px;font-size:0.8em;color:#aaa">
            <strong style="color:#00ff88">High (70+):</strong> "Stop-loss at $142 (-5%). Take profit at $165 (+10%). Thesis invalidated if Fed raises rates or Q1 earnings miss by &gt;10%. Risk/reward: 2:1."<br>
            <strong style="color:#ff4444">Low (&lt;30):</strong> "Buying because I think it will go up." (no exit plan)
          </div>
        </div>
      </div>
