/**
 * Benchmark V34 Dashboard — 28-Dimension AI Trading Benchmark
 *
 * NEW in v34: Reasoning Traceability + Adversarial Coherence
 *
 * Sections:
 ...
 * 6. Footer
 *
 * GET /benchmark-v34 — Full benchmark dashboard
 */
...
export const benchmarkV34Routes = new Hono();
...
const DIMS: DimDef[] = [
  ...
  { key: "reasoningTraceability", label: "Traceability", cat: "Reasoning Quality", isNew: true },
  { key: "adversarialCoherence", label: "Adversarial Coherence", cat: "Reasoning Quality", isNew: true },
...
benchmarkV34Routes.get("/", (c) => {
  ...
  const feedItems = trades.slice(0, 10).map((t) => {
    ...
    <div class="feed-item" style="border-left-color:${gc}">
      ...
      <div style="display:flex;gap:12px;font-size:0.72em;color:#666;flex-wrap:wrap">
        ...
        <span><span class="dot" style="background:${colorizeScore(t.reasoningTraceabilityScore)}"></span>Trace ${t.reasoningTraceabilityScore.toFixed(0)}</span>
        <span><span class="dot" style="background:${colorizeScore(t.adversarialCoherenceScore)}"></span>Adv ${t.adversarialCoherenceScore.toFixed(0)}</span>
  ...
  /* API table */
  const apis = [
    ["/api/v1/benchmark-v34/leaderboard", "Ranked agents by 28-dimension composite score (JSON)"],
    ["/api/v1/benchmark-v34/trade-grades", "Individual trade quality assessments with traceability + adversarial scores"],
    ["/api/v1/benchmark-v34/dimensions", "All 28 dimension definitions and weights"],
    ["/api/v1/benchmark-v34/traceability/:agentId", "Per-agent reasoning traceability analysis"],
    ["/api/v1/benchmark-v34/adversarial/:agentId", "Per-agent adversarial coherence analysis"],
    ["/api/v1/benchmark-v34/reasoning-profile", "Cross-agent reasoning quality comparison"],
    ["/api/v1/benchmark-v34/export/jsonl", "Full dataset export (JSONL for HuggingFace)"],
    ["/api/v1/benchmark-v34/export/csv", "Full dataset export (CSV)"],
    ["/api/v1/benchmark-v34/health", "Engine health check"],
  ];
...
<div class="wrap">
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;margin-bottom:22px">
    ...
    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
      ...
      <a href="/api/v1/benchmark-v34/export/jsonl" style="background:#1a1a2e;padding:8px 14px;border-radius:8px;font-size:0.82em;text-decoration:none">JSONL</a>
      <a href="/api/v1/benchmark-v34/export/csv" style="background:#1a1a2e;padding:8px 14px;border-radius:8px;font-size:0.82em;text-decoration:none">CSV</a>
    </div>
  </div>
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;margin-bottom:22px">
    ...
    <div class="stat"><div class="stat-v">${lr ? lr.avgTraceability.toFixed(0) : "--"}</div><div class="stat-l">Avg Trace <span class="new-badge">NEW</span></div></div>
    <div class="stat"><div class="stat-v">${lr ? lr.avgAdversarialCoherence.toFixed(0) : "--"}</div><div class="stat-l">Avg Adversarial <span class="new-badge">NEW</span></div></div>
  </div>
  ...
  <div class="card">
    ...
    <p style="color:#555;font-size:0.78em;margin-bottom:14px">Per-agent scores grouped by category. <span class="new-badge">NEW</span> = v34 dimensions.</p>
  ...
  <div class="grid2">
    <div class="card">
      ...
      <p style="color:#555;font-size:0.75em;margin-bottom:8px">Last 10 trades | <a href="/api/v1/benchmark-v34/trade-grades">Full JSON</a></p>
    ...
    <div>
      <div class="card" style="margin-bottom:18px">
        ...
        <div style="margin-bottom:14px">
          <h3 style="color:#ffd700;font-size:0.92em">Reasoning Traceability <span class="new-badge">NEW</span></h3>
          <p style="color:#999;font-size:0.82em;margin:5px 0">
            Can each claim in the agent's reasoning be traced back to a cited
            data source? Measures source-attribution density, claim-source pairing,
            orphan claim detection, evidence chain completeness, and quantitative backing.
          </p>
          <div style="background:#111;border-radius:6px;padding:8px;font-size:0.8em;color:#aaa">
            <strong style="color:#00ff88">High (70+):</strong> "Based on Jupiter price data, NVDA is at $148.50 (+3.2% 24h). Combined with the volume spike I found via search..."<br>
            <strong style="color:#ff4444">Low (&lt;30):</strong> "NVDA will definitely go up. I'm buying."
          </div>
        </div>
        <div>
          <h3 style="color:#ffd700;font-size:0.92em">Adversarial Coherence <span class="new-badge">NEW</span></h3>
          <p style="color:#999;font-size:0.82em;margin:5px 0">
            Does reasoning hold up against contrary evidence? Measures
            counterargument acknowledgment, conflicting-data handling,
            risk-factor integration, conviction justification, and scenario planning.
          </p>
          <div style="background:#111;border-radius:6px;padding:8px;font-size:0.8em;color:#aaa">
            <strong style="color:#00ff88">High (70+):</strong> "Despite bearish RSI readings, the earnings catalyst outweighs short-term technicals. If earnings miss, I'd exit at $140."<br>
            <strong style="color:#ff4444">Low (&lt;30):</strong> "Everything points to buy. No risks."
          </div>
        </div>
      </div>
