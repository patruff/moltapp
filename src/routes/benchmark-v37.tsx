/**
 * Benchmark V37 Dashboard — 34-Dimension AI Trading Benchmark
 *
 ...
 * 6. Footer
 *
 * GET /benchmark-v37 — Full benchmark dashboard
 */
...
export const benchmarkV37Routes = new Hono();
...
const DIMS: DimDef[] = [
  ...
  { key: "informationAsymmetry", label: "Info Asymmetry", cat: "Reasoning Quality" },
  { key: "temporalReasoningQuality", label: "Temporal Reasoning", cat: "Reasoning Quality" },
  { key: "reasoningAuditability", label: "Auditability", cat: "Reasoning Quality" },
  { key: "decisionReversibility", label: "Reversibility", cat: "Reasoning Quality" },
  { key: "reasoningComposability", label: "Composability", cat: "Reasoning Quality", isNew: true },
  { key: "strategicForesight", label: "Strategic Foresight", cat: "Reasoning Quality", isNew: true },
...
benchmarkV37Routes.get("/", (c) => {
  ...
  /* API table */
  const apis = [
    ["/api/v1/benchmark-v37/leaderboard", "Ranked agents by 34-dimension composite score (JSON)"],
    ["/api/v1/benchmark-v37/trade-grades", "Individual trade quality assessments with composability + foresight"],
    ["/api/v1/benchmark-v37/dimensions", "All 34 dimension definitions and weights"],
    ["/api/v1/benchmark-v37/composability/:agentId", "Per-agent reasoning composability analysis"],
    ["/api/v1/benchmark-v37/foresight/:agentId", "Per-agent strategic foresight analysis"],
    ["/api/v1/benchmark-v37/reasoning-profile", "Cross-agent reasoning quality comparison"],
    ["/api/v1/benchmark-v37/export/jsonl", "Full dataset export (JSONL for HuggingFace)"],
    ["/api/v1/benchmark-v37/export/csv", "Full dataset export (CSV)"],
    ["/api/v1/benchmark-v37/health", "Engine health check"],
  ];
...
<div class="wrap">
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;margin-bottom:22px">
    ...
    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
      ...
      <a href="/api/v1/benchmark-v37/export/jsonl" style="background:#1a1a2e;padding:8px 14px;border-radius:8px;font-size:0.82em;text-decoration:none">JSONL</a>
      <a href="/api/v1/benchmark-v37/export/csv" style="background:#1a1a2e;padding:8px 14px;border-radius:8px;font-size:0.82em;text-decoration:none">CSV</a>
    </div>
  </div>
  ...
  <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:10px;margin-bottom:22px">
    ...
    <div class="stat"><div class="stat-v">${lr ? lr.avgReasoningComposability.toFixed(0) : "--"}</div><div class="stat-l">Avg Composability <span class="new-badge">NEW</span></div></div>
    <div class="stat"><div class="stat-v">${lr ? lr.avgStrategicForesight.toFixed(0) : "--"}</div><div class="stat-l">Avg Foresight <span class="new-badge">NEW</span></div></div>
  </div>
  ...
  <div class="card">
    ...
    <p style="color:#555;font-size:0.78em;margin-bottom:14px">Per-agent scores grouped by category. <span class="new-badge">NEW</span> = v37 dimensions.</p>
  ...
  <div class="grid2">
    <div class="card">
      ...
      <p style="color:#555;font-size:0.75em;margin-bottom:8px">Last 10 trades | <a href="/api/v1/benchmark-v37/trade-grades">Full JSON</a></p>
    ...
    <div>
      <div class="card" style="margin-bottom:18px">
        ...
        <div style="margin-bottom:14px">
          <h3 style="color:#ffd700;font-size:0.92em">Reasoning Composability <span class="new-badge">NEW</span></h3>
          <p style="color:#999;font-size:0.82em;margin:5px 0">
            Can the agent's reasoning be decomposed into independent, reusable modules?
            Measures argument modularity, cross-trade reasoning reuse, hierarchical argument
            structure, transferable insight detection, and synthesis quality.
          </p>
          <div style="background:#111;border-radius:6px;padding:8px;font-size:0.8em;color:#aaa">
            <strong style="color:#00ff88">High (70+):</strong> "Thesis: NVDA is undervalued. Sub-thesis 1: AI chip demand growing 40% YoY (source: IDC). Sub-thesis 2: Gross margins expanding due to CUDA moat. Sub-thesis 3: Data center TAM underestimated. Synthesis: All three factors compound."<br>
            <strong style="color:#ff4444">Low (&lt;30):</strong> "NVDA looks good. Buying."
          </div>
        </div>
        <div>
          <h3 style="color:#ffd700;font-size:0.92em">Strategic Foresight <span class="new-badge">NEW</span></h3>
          <p style="color:#999;font-size:0.82em;margin:5px 0">
            Does the agent anticipate second-order effects and plan multiple moves ahead?
            Measures scenario planning depth, conditional strategy articulation, opportunity
            cost awareness, multi-step trade sequencing, and contingency breadth.
          </p>
          <div style="background:#111;border-radius:6px;padding:8px;font-size:0.8em;color:#aaa">
            <strong style="color:#00ff88">High (70+):</strong> "If Q1 earnings beat, institutional buying pushes past $150 resistance. In that scenario, I add 20% to position. If earnings miss, I exit entirely as thesis is invalidated. Opportunity cost: could buy AMD instead but NVDA has stronger moat."<br>
            <strong style="color:#ff4444">Low (&lt;30):</strong> "I think the stock will go up."
          </div>
        </div>
      </div>
