/**
 * Benchmark V36 API — 32-Dimension AI Trading Benchmark
 *
 * Researcher-facing API for the v36 benchmark framework.
 ...
 * - GET /dimensions                — All 32 dimension definitions and weights
 * - GET /auditability/:agentId   — Reasoning auditability analysis (NEW)
 * - GET /reversibility/:agentId       — Decision reversibility analysis (NEW)
 * - GET /info-asymmetry/:agentId   — Information asymmetry analysis (inherited)
 * - GET /temporal/:agentId         — Temporal reasoning analysis (inherited)
...
// ---------------------------------------------------------------------------
// GET /leaderboard — Ranked agents by 32-dimension composite score
// ---------------------------------------------------------------------------

benchmarkV36ApiRoutes.get("/leaderboard", (c) => {
  const scores = getAgentScores();
  const sorted = [...scores].sort((a, b) => b.compositeScore - a.compositeScore);

  return c.json({
    ok: true,
    benchmark: "MoltApp v36",
...
benchmarkV36ApiRoutes.get("/trade-grades", (c) => {
...
benchmarkV36ApiRoutes.get("/dimensions", (c) => {
    ...
    categories: [
      ...
      },
      {
        name: "Reasoning Quality", weight: 0.50, dimensions: [
          ...
          { key: "reasoningAuditability", label: "Reasoning Auditability", weight: weights.reasoningAuditability, description: "Can every claim be independently verified? Evidence specificity, falsifiability, verifiable reference density, audit trail completeness", isNew: true },
          { key: "decisionReversibility", label: "Decision Reversibility", weight: weights.decisionReversibility, description: "Does the agent plan for when its thesis breaks? Exit conditions, invalidation criteria, contingency planning, risk-reward framing", isNew: true },
        ],
      },
      {
...
benchmarkV36ApiRoutes.get("/auditability/:agentId", (c) => {
...
benchmarkV36ApiRoutes.get("/reversibility/:agentId", (c) => {
  const agentId = c.req.param("agentId");
  const trades = getTradeGradesByAgent(agentId, 100);
...
benchmarkV36ApiRoutes.get("/reasoning-profile", (c) => {
  const scores = getAgentScores();
  ...
  return c.json({ ok: true, profiles, dimensions: getDimensionCount(), version: "36.0" });
});
...
benchmarkV36ApiRoutes.get("/justification/:agentId", (c) => {
  const agentId = c.req.param("agentId");
...
benchmarkV36ApiRoutes.get("/predictions", (c) => {
  ...
  return c.json({ ok: true, predictions, count: predictions.length, version: "36.0" });
});
...
benchmarkV36ApiRoutes.get("/consensus", (c) => {
...
benchmarkV36ApiRoutes.get("/export/jsonl", (c) => {
  ...
  return new Response(lines.join("\n") + "\n", {
    headers: {
      ...
      "Content-Disposition": "attachment; filename=molt-benchmark-v36.jsonl",
    },
  });
});
...
benchmarkV36ApiRoutes.get("/export/csv", (c) => {
  const scores = getAgentScores();
  const header = "agent_id,agent_name,provider,model,composite_score,tier,trade_count,reasoning_auditability,decision_reversibility\n";
  const rows = scores.map((s) =>
    `${s.agentId},${s.agentName},${s.provider},${s.model},${s.compositeScore},${s.tier},${s.tradeCount},${s.dimensions.reasoningAuditability},${s.dimensions.decisionReversibility}`,
  ).join("\n");

  return new Response(header + rows + "\n", {
    headers: {
      "Content-Type": "text/csv",
      "Content-Disposition": "attachment; filename=molt-benchmark-v36-agents.csv",
    },
  });
});
...
benchmarkV36ApiRoutes.get("/health", (c) => {
  const scores = getAgentScores();
  ...
  return c.json({
    ...
    version: getBenchmarkVersion(),
    dimensions: getDimensionCount(),
    newDimensions: ["reasoning_auditability", "decision_reversibility"],
